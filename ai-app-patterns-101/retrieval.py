...
class SimpleVectorStore:
    def __init__(self, items: list[dict[str, str]], model_name: str = "sentence-transformers/all-MiniLM-L6-v2") -> None:
        self.items = items
        self.encoder = SentenceTransformer(model_name)
        embeddings = self.encoder.encode([item["content"] for item in items], normalize_embeddings=True)
