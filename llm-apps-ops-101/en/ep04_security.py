...
class InputValidator:
    ...
    def validate(self, text: str) -> ValidationResult:
        ...
        if len(text) > self.max_length:
            return ValidationResult(False, f'Input is too long. Maximum length is {self.max_length} characters.')
        lowered = text.lower()
        for pattern in BLOCKED_PATTERNS:
            if re.search(pattern, lowered):
                return ValidationResult(False, 'Potential prompt injection pattern detected.')
    ...
    def redact(self, text: str) -> str:
        ...
        for name, pattern in SENSITIVE_PATTERNS.items():
            ...
            if replaced != filtered:
                logger.info('Sensitive data was removed from the output.', extra={'payload': {'kind': name}})
            filtered = replaced
        return filtered
...
def demo() -> None:
    ...
    print(validation)
    print(filter_.redact('Contact admin@example.com or call +1 415 555 0100 for support.'))
