...
class InstrumentedLLM:
    ...
    def invoke(self, system_prompt: str, user_prompt: str) -> tuple[str, LLMCallRecord]:
        ...
        logger.info('LLM 호출을 시작합니다.', extra={'payload': {'call_id': record.call_id, 'model': self.model}})
        try:
            ...
            logger.info('LLM 호출이 완료되었습니다.', extra={'payload': record.to_payload()})
            return result.text, record
        except Exception as exc:
            record.success = False
            record.error = str(exc)
            logger.exception('LLM 호출이 실패했습니다.', extra={'payload': record.to_payload()})
            raise


def sample_monitoring_session() -> None:
    llm = InstrumentedLLM()
    system_prompt = '당신은 운영 대시보드에 짧은 상태 요약을 남기는 SRE 어시스턴트입니다.'
