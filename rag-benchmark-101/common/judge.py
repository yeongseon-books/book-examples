...
def parse_score(raw: str, default: float = 3.0) -> JudgeScore:
    try:
        ...
        return JudgeScore(float(payload.get("score", default)), str(payload.get("reason", "")))
    except Exception:
        return JudgeScore(default, "")


def judge_with_groq(system_prompt: str, user_prompt: str, model: str = "llama-3.1-8b-instant") -> JudgeScore:
...
def evaluate_generation(question: str, context: str, answer: str, language: str) -> dict[str, JudgeScore]:
    if language == "ko":
        faithfulness_user = (
