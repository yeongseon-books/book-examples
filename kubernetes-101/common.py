...
class PodValidator:
    def validate(self, pod: dict[str, Any]) -> list[str]:
        ...
        for c in containers:
            ...
            if not limits:
                errors.append(f"container {c.get('name', 'unknown')} missing resource limits")
...
class ServiceResolver:
    ...
    def match_pods(self, service: dict[str, Any], pods: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
class ConfigMapSecretLoader:
    def load_env(self, pod: dict[str, Any], configmaps: dict[str, dict[str, str]], secrets: dict[str, dict[str, str]]) -> dict[str, str]:
        env: dict[str, str] = {}
...
class VolumeBinder:
    def bind(self, pvs: list[dict[str, Any]], pvc: dict[str, Any]) -> str | None:
        req = pvc.get('spec', {}).get('resources', {}).get('requests', {}).get('storage', '0Gi')
        req_size = int(req.replace('Gi', ''))
        req_sc = pvc.get('spec', {}).get('storageClassName')
        req_modes = set(pvc.get('spec', {}).get('accessModes', []))
        for pv in pvs:
            cap = int(pv.get('spec', {}).get('capacity', {}).get('storage', '0Gi').replace('Gi', ''))
...
class HPAController:
    def desired_replicas(self, current_replicas: int, current_cpu: int, target_cpu: int, min_replicas: int, max_replicas: int) -> int:
        if target_cpu <= 0:
