...
def train_models(seed: int = 42) -> dict[str, float]:
    ...
    rf = RandomForestClassifier(n_estimators=120, random_state=seed).fit(X_train, y_train)
    lr_acc = float(lr.score(X_test, y_test))
    rf_acc = float(rf.score(X_test, y_test))
    cv_mean = float(cross_val_score(rf, X_train, y_train, cv=5, scoring="accuracy").mean())
