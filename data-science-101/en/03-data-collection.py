...
def generate_collected_data(
    ...
    events = pd.DataFrame(
        {
            ...
            "event_type": rng.choice(
                ["login", "view", "purchase"], size=n_users * 5, p=[0.5, 0.4, 0.1]
            ),
    ...
    merged = users.merge(
        events.groupby("user_id").size().rename("event_count"), on="user_id", how="left"
    ).merge(
        tx.groupby("user_id")["amount"].sum().rename("total_amount"),
        on="user_id",
        how="left",
    )
