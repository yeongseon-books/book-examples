...
def main() -> None:
    ...
    response = client.chat.completions.create(
        ...
        temperature=0.5,
        messages=[
            {'role': 'system', 'content': '당신은 데이터셋 작성 도우미입니다. 결과는 반드시 JSON 배열만 출력하세요.'},
            {'role': 'user', 'content': '고객 지원 챗봇 파인튜닝용 instruction-response 데이터 5개를 JSON 배열로 생성해 주세요. 각 항목은 instruction, response, category 필드를 가져야 합니다. 한국어로 작성하세요.'},
        ],
    )
