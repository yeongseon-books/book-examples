...
def main() -> None:
    ...
    print('동일한 사용자 요청을 베이스 프롬프트와 도메인 특화 프롬프트로 각각 실행합니다.')
    print()
    print('사용자 요청')
    print(indent(user_request, prefix='  '))
    print()

    base_response = request_completion(client, '당신은 일반적인 한국어 비서입니다. 사용자의 요청에 짧고 무난하게 답하세요.', user_request)
    tuned_response = request_completion(client, '당신은 B2B SaaS 운영 공지 작성에 특화된 모델입니다. 제품명 PulseBoard, 고객 대상 톤, 상태 요약, 다음 업데이트 시각, 재발 방지 의지를 반영해 구조화된 공지를 작성하세요.', user_request)
