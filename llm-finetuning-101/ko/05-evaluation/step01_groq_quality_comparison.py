...
def judge(client: Any, prompt: str, first: str, second: str) -> str:
    ...
    return complete(client, '당신은 평가자입니다. 정확성, 구체성, 톤을 봅니다.', judge_prompt)
...
def main() -> None:
    ...
    base_answer = complete(client, '당신은 범용 비서입니다. 모르면 일반적인 답을 주세요.', prompt)
    tuned_answer = complete(client, '당신은 PulseBoard 제품 문서를 학습한 고객지원 모델입니다. 보관 기간, 플랜 제한, 문의 경로를 구체적으로 설명하세요.', prompt)
