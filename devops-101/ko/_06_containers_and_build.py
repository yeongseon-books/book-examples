...
def lint_dockerfile(text: str) -> dict[str, object]:
    ...
    if not any('--no-cache-dir' in line for line in lines if line.startswith('RUN ') and 'pip install' in line):
        warnings.append('Use --no-cache-dir for pip install')
    if not any(line.startswith('USER ') and 'root' not in line.lower() for line in lines):
        warnings.append('Run container as non-root user')

    return {'warnings': warnings, 'has_multistage': has_multistage, 'estimated_size_mb': estimate_image_size(lines)}
